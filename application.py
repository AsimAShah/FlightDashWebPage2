...
def compute_data_choice_1(df):
    # Cancellation Category Count - bar graph 
    bar_data = df.groupby(['Month','CancellationCode'])['Flights'].sum().reset_index()
    # Average flight time by reporting airline - line plot
    line_data = df.groupby(['Month','Reporting_Airline'])['AirTime'].mean().reset_index()
    # Diverted Airport Landings - pie chart
    div_data = df[df['DivAirportLandings'] != 0.0]
    # Source state count - chloropleth map
    map_data = df.groupby(['OriginState'])['Flights'].sum().reset_index()
    # Destination state count - treemap
...
def get_graph(chart, year, children1, children2, children3, children4,children5):
        ...
        if chart == 'OPT1':
            ...
            line_fig = px.line(line_data, x='Month', y='AirTime', color='Reporting_Airline', title='Average Flight Time by Airline')
